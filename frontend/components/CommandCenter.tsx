...
import type { DemoResponse, FleetHealth, LiveResponse, SensorSummary } from "../lib/types";
...
export default function CommandCenter() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      try { const fleet = await getSensors(); setSensors(fleet.sensors); setFleetHealth(fleet.health || null); } catch { setSensors([]); setFleetHealth(null); }
    } catch (reason) {
      setLive(null);
      setDemo(null);
  ...
  const selectedRemoteForecast = selectedSensor?.status === "ONLINE" && selectedSensor.telemetry_status === "FRESH" ? selectedSensor.runtime?.forecast : undefined;
